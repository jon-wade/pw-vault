        ...
        db.controller.read({_id: id}, 'email', model).then(function(res) {
            ...
            if(res.length !==0) {
                ...
                if(hashedemail === res[0].email) {
                    ...
                    mailer.send(email, 'Password Vault Password Recovery', 'Hi there - you recently requested to reset your password. Please click this link to start the password reset process: http://pw-vault.com/change-password?id=' + id + '. Best, Password Vault')
                        .then(function(res) {
                            //successful mailing
                            resolve({
